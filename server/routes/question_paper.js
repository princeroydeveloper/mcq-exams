...
router.post('/delete', validateTeacher, [
  ...
  try {
    ...
    // Proceed further
    const requiredQP = await QuestionPaper.findOne({ _id: ObjectId(req.body.qpId), uid: ObjectId(req.user.uid) })
    if (requiredQP) {
      ...
      const deletedAnswers = await Answer.deleteMany({ paper_id: requiredQP.paper_id })
      if (deletedQp && deletedQuestions && deletedAnswers) {
...
router.post('/rename', validateTeacher, [
  body('qpId', 'Request processing error...').isLength({ min: 1 }),
  body('name', 'Name of question paper must contain at least 5 characters').isLength({ min: 5 })
], async (req, res) => {
  try {
    // Finds error in validation and return bad request
    const errors = validationResult(req)
    ...
    const requiredQP = await QuestionPaper.findOne({ _id: ObjectId(req.body.qpId), uid: ObjectId(req.user.uid) })
    if (requiredQP) {
      const renamedQP = await QuestionPaper.findOneAndUpdate({ _id: ObjectId(req.body.qpId), uid: ObjectId(req.user.uid) }, { name: req.body.name.trim() }, { new: true })
      if (renamedQP) {
        return res.json({ success: 'Question paper renamed successfully!', db_value: renamedQP.name })
      }
      return res.status(400).json({ error: 'Error occurred while renaming paper.' })
    }
    return res.status(400).json({ error: 'Cannot find question paper.' })
  } catch (error) {
    console.error(error)
  ...
  try {
    ...
    if (requiredQP) {
      ...
      if (all_questions_ids) {
        return res.json({ total: all_questions_ids, paper_id: requiredQP.paper_id, duration: requiredQP.duration, name: requiredQP.name })
      }
      return res.status(400).json({ error: 'Cannot find question paper.' })
    }
    return res.status(400).json({ error: 'Cannot find question paper.' })
